...
public:
    ...
    // Вставляет элемент value после элемента, на который указывает position.
    // Возвращает итератор на вставленный элемент
    // Если при создании элемента будет выброшено исключение, список останется в прежнем состоянии
    Iterator InsertAfter(ConstIterator position, const Type& value) {
        assert(position.node_ != nullptr);
        position.node_->next_node = new Node(value, position.node_->next_node);
        ++size_;
        return Iterator{position.node_->next_node };
    }

    // Удаляет элемент, следующий за position.
    // Возвращает итератор на элемент, следующий за удалённым
    Iterator EraseAfter(ConstIterator position) noexcept {
        assert(!IsEmpty());
        assert(position.node_ != nullptr);
        auto temp = position.node_->next_node->next_node;
        delete position.node_->next_node;
        position.node_->next_node = temp;
        --size_;
        return Iterator{position.node_->next_node };
    }
...
private:
    ...
    void Assign(Iterator beg, Iterator end) {
        ...
        auto node_ptr = &temp.head_.next_node;
        while (beg != end) {
            assert(*node_ptr == nullptr);
            *node_ptr = new Node(*beg, nullptr);
            ++temp.size_;
